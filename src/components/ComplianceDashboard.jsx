    ...
    <div className="space-y-5">
      ...
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4  border shadow-sm ${isLight ? 'bg-white border-slate-200 text-slate-900' : 'bg-[#111622] border-slate-800 text-slate-100'
        ...
        {canDownload ? (
          <button
            ...
            className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-4 py-2.5 flex items-center space-x-2 transition-all shadow-md shadow-emerald-600/20 self-start sm:self-center shrink-0"
          >
          ...
          <button
            onClick={() => handleExportPDF(null)}
            className="bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs font-bold px-4 py-2.5 flex items-center space-x-2 border border-slate-300 dark:border-slate-700 cursor-not-allowed self-start sm:self-center shrink-0"
            title="🔒 Access Restricted: Requires Auditor or Admin Privileges"
          >
            <Lock className="w-4 h-4 text-amber-500" />
            <span>🔒 Export CPCB Form IV report (Auditor Only)</span>
          </button>
        )}
      </div>
      ...
      <div className={`p-4  border shadow-sm ${isLight ? 'bg-white border-slate-200' : 'glass-panel border-slate-800'
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center text-xs">
          <div className={`p-2.5  border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#0b0f19] border-slate-800'}`}>
            <div className="w-6 h-6  bg-cyan-100 text-cyan-700 dark:bg-cyan-500/20 dark:text-cyan-400 font-bold text-xs flex items-center justify-center mx-auto mb-1">1</div>
          ...
          <div className={`p-2.5  border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#0b0f19] border-slate-800'}`}>
            <div className="w-6 h-6  bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400 font-bold text-xs flex items-center justify-center mx-auto mb-1">2</div>
          ...
          <div className={`p-2.5  border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#0b0f19] border-slate-800'}`}>
            <div className="w-6 h-6  bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 font-bold text-xs flex items-center justify-center mx-auto mb-1">3</div>
          ...
          <div className={`p-2.5  border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#0b0f19] border-slate-800'}`}>
            <div className="w-6 h-6  bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400 font-bold text-xs flex items-center justify-center mx-auto mb-1">4</div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
          <div>
            <h3 className={`text-sm font-bold flex items-center gap-2 ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>
              ...
              CPCB Bag Traceability & Immutable Audit Logs
            </h3>
            <p className={`text-[11px] ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>Cryptographically hash-chained digital records for medical board audit</p>
          </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className={`border-b font-semibold ${isLight ? 'bg-slate-50 text-slate-600 border-slate-200' : 'bg-[#090d16] text-slate-400 border-slate-800'
                ...
                <th className="py-2.5 px-3">SHA-256 Hash Verified</th>
                  ...
                  <td className="py-3 px-3 font-mono text-[10px]">
                    {canVerifyHashes ? (
                      <span className="flex items-center space-x-1 text-emerald-600 dark:text-emerald-400 font-bold">
                        ...
                        <span>{log.hashChain ? log.hashChain.substring(0, 12) + '...' : 'VERIFIED'}</span>
                      </span>
                    ) : (
                      <span className="flex items-center space-x-1 text-slate-400 dark:text-slate-500 font-semibold" title="🔒 Hash Verification Requires Auditor Privileges">
                        <Lock className="w-3 h-3 text-amber-500" />
                        <span>SHA256-••••-RESTRICTED</span>
                      </span>
                    )}
                  </td>
                  <td className="py-3 px-3">
                  ...
                  <td className="py-3 px-3 text-right flex items-center justify-end space-x-1.5">
                    ...
                    {canDownload ? (
                      <button
                        ...
                        className="p-1.5  border bg-emerald-600 text-white hover:bg-emerald-500 border-emerald-600 transition-colors shadow-sm"
                      ...
                      <button
                        onClick={() => handleExportPDF(log)}
                        className="p-1.5  border bg-slate-200 text-slate-400 dark:bg-slate-800 dark:text-slate-500 border-slate-300 dark:border-slate-700 cursor-not-allowed"
                        title="🔒 Access Restricted: Requires Auditor or Admin Privileges"
                      >
          ...
          <div className={`border  w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] ${isLight ? 'bg-white border-slate-300' : 'bg-[#121824] border-slate-700'
            ...
            <div className={`px-5 py-3 border-b flex items-center justify-between ${isLight ? 'bg-slate-100 border-slate-200 text-slate-900' : 'bg-[#090d16] border-slate-800 text-slate-100'
              ...
              <button
                ...
                className={`p-1  ${isLight ? 'text-slate-500 hover:text-slate-800' : 'text-slate-400 hover:text-slate-200'}`}
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs bg-white text-slate-900 font-sans">
              ...
              {selectedReportLog?.cpcbStatus === 'FLAGGED' && (
                <div className="p-3.5  border-2 border-rose-400 bg-rose-50 text-rose-900 space-y-1">
              ...
              {canVerifyHashes ? (
                <div className="bg-slate-900 text-cyan-300 p-3  font-mono text-[10px] space-y-1">
                ...
                <div className="bg-slate-100 border border-slate-300 text-slate-700 p-3  font-sans text-[11px] flex items-center space-x-2">
            ...
            <div className={`px-5 py-3 border-t flex justify-end space-x-3 ${isLight ? 'bg-slate-100 border-slate-200' : 'bg-[#090d16] border-slate-800'
              ...
              {canDownload ? (
                ...
                <button
                  ...
                  className="bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs font-semibold px-4 py-1.5  flex items-center space-x-1.5 border border-slate-300 dark:border-slate-700 cursor-not-allowed"
                  title="🔒 Access Restricted: Requires Auditor or Admin Privileges"
                >
                  <Lock className="w-3.5 h-3.5 text-amber-500" />
