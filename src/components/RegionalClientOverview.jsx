...
import { REGIONAL_HOSPITALS, REGIONS, UMBRELLA_GROUPS } from '../data/mockData';
import { Building2, Cpu, ShieldAlert, ArrowRight, MapPin, Search, CheckCircle2, Phone, Calendar, Award, ChevronLeft, Building, ArrowLeft } from 'lucide-react';

export default function RegionalClientOverview({ onSelectHospital, isLight, selectedGroup, setSelectedGroup }) {
  const [searchTerm, setSearchTerm] = useState('');
  const currentRegion = REGIONS[0]; // Assigned region: Bangalore South Zone (50km)

  // Filter groups
  const filteredGroups = UMBRELLA_GROUPS.filter(g =>
    g.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Filter hospitals if a group is selected
  const filteredHospitals = selectedGroup
    ? REGIONAL_HOSPITALS.filter(
        (h) => h.groupId === selectedGroup.id &&
      ...
      <div className={`p-5 border ${isLight ? 'bg-white border-slate-200/80 text-slate-900' : 'bg-[#111622] border-slate-800 text-slate-100'
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={selectedGroup ? "Search hospital or locality..." : "Search hospital chains..."}
              value={searchTerm}
      ...
      {!selectedGroup && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className={`p-4 border ${isLight ? 'bg-white border-slate-200/80 text-slate-900' : 'bg-[#111622] border-slate-800 text-slate-100'
            ...
            <div className="mt-2">
              <span className="text-3xl font-black text-emerald-600">30 Units</span>
              <p className="text-xs text-slate-500 mt-1">16 Running • 13 Idle • 1 Alarm</p>
            </div>
          </div>
      ...
      {!selectedGroup ? (
        <div className="space-y-3">
          <h3 className={`text-xs font-bold uppercase tracking-wider ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
            Select an Umbrella Chain / Group ({filteredGroups.length})
          </h3>
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h4 className="text-base font-bold text-slate-900 dark:text-slate-100">{group.name}</h4>
                  </div>
                  
                  <div className="text-right shrink-0">
                    <span className="text-2xl font-black text-emerald-600 dark:text-emerald-400">{group.overallCompliance}%</span>
                    <span className="text-[10px] font-bold text-slate-400 block">Overall Compliance</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 my-3.5 p-3 bg-slate-50 dark:bg-[#090d16] border border-slate-200/60 dark:border-slate-800 text-center text-xs">
                  <div>
                    <span className="text-slate-500 text-[10px] block">Regional Branches</span>
                    <strong className="text-slate-800 dark:text-slate-200 font-bold">{REGIONAL_HOSPITALS.filter(h => h.groupId === group.id).length} Hospitals</strong>
                  </div>
                  <div>
                    <span className="text-slate-500 text-[10px] block">National Devices</span>
                    <strong className="text-slate-800 dark:text-slate-200 font-bold">{group.totalDevices} Installed</strong>
                  </div>
                  <div>
                    <span className="text-slate-500 text-[10px] block">National Waste</span>
                    <strong className="text-slate-800 dark:text-slate-200 font-bold">{group.monthlyTreatedTons} Tons/mo</strong>
                  </div>
                </div>

                <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
                  <span className="text-slate-500 text-[11px]">Click to view regional branches</span>
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-3 mb-4">
            <button 
              ...
              className={`p-2 border transition-colors ${
                isLight ? 'bg-white border-slate-200 hover:bg-slate-100 text-slate-700' : 'bg-[#090d16] border-slate-700 hover:bg-slate-800 text-slate-300'
              }`}
              title="Back to Chains"
            >
              <ArrowLeft className="w-4 h-4" />
            </button>
            <h3 className={`text-base font-bold flex items-center space-x-2 ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>
              <Building className="w-4 h-4 text-cyan-600" />
              <span>{selectedGroup.name} - Regional Branches</span>
            </h3>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            {filteredHospitals.length === 0 && (
              <div className={`col-span-2 p-10 text-center border border-dashed ${isLight ? 'border-slate-300 text-slate-500' : 'border-slate-700 text-slate-400'}`}>
                <p className="text-sm font-bold mb-1">No Hospitals Found in Region</p>
                <p className="text-xs">There are currently no connected hospitals in this region for {selectedGroup.name}.</p>
              </div>
            )}
          </div>
        </div>
